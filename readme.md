# 개요 
![로고1](Documents/로고1.png)
 
MatZip은 Microsoft사에서 제공하는 Bing Maps API를 활용하여 나만의 맛집 리스트를 
저장하고 다른 사람과 의견을 공유할 수 있는 프로그램입니다.

# 사용기술
### 언어
* C# 3.0+
### 프레임워크
* .NET Framework 4.8
* Entity Framework 6.4.4
* Winform
### 데이터베이스
* MSSQL Server 2019
### Third Party Control
* DevExpress Winform  

# 데이터베이스 설계
![D B](Documents/DB.png)

* 구성요소 중 코드성 테이블로 만들 수 있는 것을 Code 테이블과 CodeCategory 테이블을
만들어 DB의 복잡도를 줄었습니다. 
* 외래키만 가지고 있는 Mapping Table과 같은 경우에는 ADO.NET을 생성할 때 포함되지 않아 dummy 속성을 추가했습니다.

# 설명
# 1. 지도
* 참고: 마우스 휠을 위로 올리면 Zoom In이 되고, 아래로 내리면 Zoom Out이 됩니다.

### 맛집 등록(필수 입력 사항 / 선택 입력 사항)
### 1-1) 등록할 지점 선택
![맛집추가](Documents/맛집추가.png)

* 마우스 휠을 이용하여 적절히 Zoom In, Zoom Out을 한 다음 맛집으로 등록할 곳에 마우스 커서를
올리고 Double Click을 합니다. 
* "추가하시겠습니까?"라는 MessagaBox가 뜨고 추가하려면 'Yes' or 'No'중에서 Yes를 누릅니다.


### 1-2) 가게명과 음식의 종류 기재 (필수 입력)
![About가게](Documents/about가게.png)

* 사진 아이콘을 클릭하면 해당 음식점과 관련된 이미지를 업로드 할 수 있습니다.

* 이미지 업로드 하지 않을 시, 기본 이미지가 삽입됩니다.
상호명과 음식 종류를 입력합니다. (한식 / 중식 / 일식 / 양식)

### 1-3) 영업시간, 휴무일, 메뉴 이름, 가격 기입 (선택 입력)
![가게정보(맘스)](Documents/가게정보(맘스).png)

* 이 부분은 유저가 선택해서 입력하는 부분입니다.
유저는 가게의 영업시간, 휴무일, 메뉴 이름과 가격을 입력합니다.

### 1-4) 리뷰 등록
![리뷰 작성 버튼](Documents/리뷰_작성_버튼.png)
* 유저는 리뷰 작성하기 버튼을 눌러서 리뷰를 남길 수 있습니다.

![리뷰 전체 화면](Documents/리뷰_전체_화면.png)
* 리뷰 입력 화면에서 사진, 코멘트와 평점을 입력할 수 있습니다.

![리뷰 사진 등록](Documents/리뷰_사진_등록.png)
* 사진 아이콘을 마우스로 클릭하면 원하는 이미지를 등록할 수 있습니다.

![리뷰 평점 입력](Documents/리뷰_평점_입력.png)
* 1점부터 5점까지 평점을 선택할 수 있습니다.

![리뷰 등록 완료](Documents/리뷰_등록_완료.png)
* 사진 등록, 코멘트 입력과 평점을 입력 후 완료 버튼을 누르면 새로운 리뷰가 등록됩니다.

### 1-5) 맛집 삭제
![맛집삭제](Documents/맛집삭제.png)
* 마우스 휠을 이용하여 적절히 Zoom In, Zoom Out을 한 다음 내가 등록한 맛집 중에 삭제할 MapPushPin에 마우스 커서를 올리고 Delete 버튼을 누르면
* "삭제하시겠습니까?"라는 MessagaBox가 뜨고 삭제하려면 'Yes' or 'No'중에서 Yes를 누릅니다.

### 1-6) 맛집 정보 간략히 보기
![Thumnail](Documents/thumnail.png)
* 마우스 휠을 이용하여 적절히 Zoom In, Zoom Out을 한 다음 맛집으로 등록된 지점에 마우스 커서를 올려두면
맛집의 정보를 간략화(썸네일 형식)하여 보여줍니다. 

### 2. 로그인
![Log In](Documents/logIn.png)
* 회원가입이 된 아이디와 비밀번호를 입력하고 아직 생성하지 않았다면, 회원가입 버튼을 클릭합니다.

### 3. 회원가입
![회원가입](Documents/회원가입.png)
* 생성할 아이디, 비밀번호, 이름, 이메일 순으로 입력 후 회원가입 버튼을 클릭합니다.

##### 어려웠던 점
1. 처음에 MapPushPin을 추가할 때 단지 Map Control에 Item을 추가하면 되는 줄 알았는데 아니었습니다.
이를 해결한 방법은 Map Control에 VectorItemsLayer를 추가 후 MapItemStorage를 이용해서 MapPushPin을 Map에 추가했습니다.

2. MapPushPin을 전체 맵에서 어떻게 인식해야 할지 고민을 했습니다.
처음에는 MapItemClick 이벤트를 이용해 MapItemStorage에 저장된 모든 MapPushPin의 좌표를 탐색해서 찾았습니다.
하지만 저희의 목표는 MouseMove 이벤트로 MapPushPin에 마우스 커서만 올려도 새로운 폼을 띄우고 싶었습니다.
이를 해결하기 위해 MapHitInfo 클래스를 이용해서 마우스 커서가 MapPushPin을 인식할 수 있었습니다.

3. 리뷰에 사진을 올릴 때 어려움을 겪었습니다.
dataGridView에 DB에 저장된 사진을 올릴 때 이미지 파일의 경로만 저장되어 있기 때문에 다른 사용자가 저장된 이미지를 불러오는데 어려움이 있었습니다.
이를 해결하기 위해 이미지 파일을 byte 배열로 변환해서 DB에 저장했습니다.

##### 느낀점

* 이전까지는 Git과 같은 Version Control을 이용하지 않고 일일이 프로젝트를 알집으로 압축 후, 이어붙이는 형식으로
프로젝트를 진행했지만, Git을 쓰고 공동 작업이 훨씬 수월해지는 것을 느낄 수 있었습니다.


 

